...
public class VoiceProcessing_Microphone_Granulation extends JFrame implements PitchDetectionHandler {
	...
	private Granulator pitchShifter;
	private WaveformWriter writerEdited;
    
    public static void main(String[] args) {
        
    	// get program ready to run
    	JFrame frame = new VoiceProcessing_Microphone_Granulation();
		frame.pack();
	...
	public VoiceProcessing_Microphone_Granulation() {
    ...
    private void startIO() throws Exception {
		...
		File fileEdited = new File("Granulation.wav");
		...
		writerUnedited = new WaveformWriter(format, "Unedited.wav");
     	...
     	pitchShifter = new Granulator(sampleRate, bufferSize);
		writerEdited = new WaveformWriter(format, "Granulation.wav");
		
     	// set initial preferences for pitch shifter
     	pitchShifter.setGrainSize(100);
     	pitchShifter.setGrainInterval(50);
     	pitchShifter.setTimestretchFactor(1);
     	pitchShifter.setPitchShiftFactor(1);
     	pitchShifter.setPosition(0);
     	pitchShifter.setGrainRandomness(0);
	...
	public void handlePitch(PitchDetectionResult result, AudioEvent audioEvent) {
		...
		if (result.isPitched()) {	
			...
			// shift pitch of audio event down (if in semitone proximity to note of scale)
